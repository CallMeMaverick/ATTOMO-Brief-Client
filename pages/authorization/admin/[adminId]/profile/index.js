...
async function fetchAdmin(adminId) {
    try {
        const response = await fetch(`http://localhost:3000/user/admin/${adminId}`, {
            method: "GET",
            headers: {
...
async function updateAdmin(adminId, updatedData) {
    try {
        const response = await fetch(`http://localhost:3000/user/admin/update/${adminId}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${localStorage.getItem("token")}`
        ...
        <div className="max-w-md mx-auto mt-10">
            {editMode ? (
                ...
                <div className="bg-white p-6 rounded-lg shadow-lg">
                    ...
                    <p className="mb-2"><span className="font-bold">Status:</span> <span className="italic font-bold text-cyan-500">{admin.status}</span></p>
